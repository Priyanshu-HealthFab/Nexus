...
export function applySetup(p: SetupPayload, choice: ApplyChoice): number {
  ...
  const added: LinkedCalendar[] = choice.calendars
    ? newCalendars(p, s.linkedCalendars).map((c) => ({ id: crypto.randomUUID(), name: c.name || 'Calendar', url: c.url, color: c.color, enabled: c.enabled }))
    : [];
  patchSettings({
    ...(choice.prefs ? p.prefs : {}),
    ...(added.length ? { linkedCalendars: [...s.linkedCalendars, ...added] } : {}),
